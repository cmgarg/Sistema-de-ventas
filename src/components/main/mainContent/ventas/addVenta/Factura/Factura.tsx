...
const Factura: React.FC<FacturaProps> = ({ setCurrentStage, dispatch }) => {
  ...
  return (
    <div className="absolute bottom-0 top-0 right-0 left-0 flex flex-col borde items-center justify-center text-slate-50 text-lg border-slate-800 rounded-md z-50 backdrop-blur-sm">
      <div className="h-1/2 w-3/6  flex flex-col justify-center items-center relative p-2 rounded-lg bg-[#2f2f2fff] border border-gray-600">
        {routesBill === "/billForm" ? (
          <BillForm
          ...
          <div className="flex flex-col w-full flex-1 relative space-y-2">
            <div className="flex flex-1 w-full space-x-2">
              <div className="flex-1 h-full bg-gradient-to-t from-sky-800 to-sky-500 rounded-lg">
                <button
                  onClick={() => {
                    setRoutesBill("/billForm");
                  }}
                  className="h-full w-full flex items-center flex-col justify-evenly pb-5 hover:backdrop-brightness-125"
                >
                  <PiPrinter size={100} className="flex-1" />
                  <p>Generar factura</p>
                </button>
              </div>
              <div className="flex-1 h-full bg-gradient-to-t from-yellow-800 to-yellow-500 rounded-lg">
                <button
                  onClick={() => {
                    dispatch({ type: "BILL_TYPE", payload: "BUDGET" });
                  }}
                  className="h-full w-full flex flex-col justify-evenly items-center pb-5 hover:backdrop-brightness-125"
                >
